...
mod tests {
    ...
    fn test_deep_conversion_chain_resolution() {
        ...
        println!("Resolved conversions: {:?}", root_conv.resolved_conversions.is_some());
        println!("Default conversion: {:?}", root_conv.default_conversion.is_some());
        
        // For ValueToText with multiple refs, we should have both resolved conversions and default
        assert!(root_conv.resolved_conversions.is_some() || root_conv.default_conversion.is_some(), 
        ...
        if let Some(resolved_conversions) = &root_conv.resolved_conversions {
            for (idx, conv) in resolved_conversions.iter() {
                println!("Resolved conversion {}: has texts={:?}, has conversions={:?}", 
                        idx, conv.resolved_texts.is_some(), conv.resolved_conversions.is_some());
                // Each resolved conversion should have resolved its own dependencies
                assert!(conv.resolved_texts.is_some() || conv.resolved_conversions.is_some() || conv.default_conversion.is_some(),
                       "Nested conversion should have resolved dependencies");
            }
        }
        
        if let Some(default_conv) = &root_conv.default_conversion {
            println!("Default conversion: has texts={:?}, has conversions={:?}", 
                    default_conv.resolved_texts.is_some(), default_conv.resolved_conversions.is_some());
            // Default conversion should also have resolved its dependencies
            assert!(default_conv.resolved_texts.is_some() || default_conv.resolved_conversions.is_some() || default_conv.default_conversion.is_some(),
    ...
    fn test_conversion_chain_cycle_detection() {
        ...
        assert!(result.is_err(), "Cycle detection should catch the circular reference");
        
        if let Err(crate::error::MdfError::ConversionChainCycle { address }) = result {
    ...
    fn test_conversion_chain_depth_limit() {
        ...
        for i in 0..25 {
            ...
            let conv = create_test_conversion_block(
                ConversionType::ValueToText,
                vec![1.0],
                if next_offset > 0 { vec![next_offset] } else { vec![] }
            );
        ...
        if let Err(crate::error::MdfError::ConversionChainTooDeep { max_depth }) = result {
    ...
    fn test_default_conversion_resolution() {
        ...
        let mut root_conv = ConversionBlock::from_bytes(&file_data[0..]).unwrap();
        let result = root_conv.resolve_all_dependencies(&file_data);
        assert!(result.is_ok(), "Resolution with default conversion should succeed");
        ...
        assert!(root_conv.resolved_texts.is_some() || root_conv.default_conversion.is_some(),
