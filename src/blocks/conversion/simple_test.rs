...
mod simple_conversion_tests {
    ...
    fn test_simple_linear_conversion_no_references() {
        ...
        assert!(result.is_ok(), "Simple conversion resolution should succeed");

        // No resolved data should be created for simple conversions
        assert!(conversion.resolved_texts.is_none(), "Should have no resolved texts");
        assert!(conversion.resolved_conversions.is_none(), "Should have no resolved conversions");
        assert!(conversion.default_conversion.is_none(), "Should have no default conversion");
    }
    ...
    fn test_identity_conversion_no_references() {
        ...
        assert!(result.is_ok(), "Identity conversion resolution should succeed");
    ...
    fn test_value_to_text_with_non_zero_addresses() {
        ...
        println!("Resolved conversions: {:?}", conversion.resolved_conversions.is_some());
        println!("Default conversion: {:?}", conversion.default_conversion.is_some());
        
        assert!(result.is_ok(), "Text conversion resolution should succeed");

        // Should have resolved texts but no conversions
        assert!(conversion.resolved_texts.is_some(), "Should have resolved texts");
        assert!(conversion.resolved_conversions.is_none(), "Should have no resolved conversions for simple text refs");
        assert!(conversion.default_conversion.is_none(), "Should have no default conversion");
