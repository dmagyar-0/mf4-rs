...
- `js/` is the npm package (`mf4-rs`): a typed TypeScript wrapper (`js/src/`) over the generated wasm-bindgen module, adding Node convenience constructors (`Mdf.fromFile`, `Mdf.fromUrl`, `MdfIndex.fromFile`, `MdfIndex.fromUrl`) and the pluggable `RangeSource` abstraction (`FetchRangeSource`, `FileRangeSource`, `BytesRangeSource`) for lazy remote reads
